  ...
  const result = await workflow('ricky-package-checks')
    ...
    .step('prepare-package-check-context', {
      ...
      command: [
        ...
        `printf '%s\\n' 'routing=local|cloud|mcp' 'cwd=$repo_root' 'typecheck=npm run typecheck' 'test=npm run test' 'invocation=node workflows/generated/ricky-package-checks.ts' 'environment=node >=18, npm workspaces' 'mcp_boundary=read-only health check, no mutations' > "$repo_root/${artifactRoot}/routing.txt"`,
    ...
    .step('typecheck-packages', {
      ...
      command: [
        ...
        `mkdir -p "$repo_root/${artifactRoot}"`,
        'cd "$repo_root"',
        `npm run typecheck > "$repo_root/${artifactRoot}/typecheck.log" 2>&1`,
        `printf '%s\\n' 'TYPECHECK_PASS' > "$repo_root/${artifactRoot}/typecheck.status"`,
        'echo TYPECHECK_PASS',
      ].join(' && '),
      captureOutput: true,
      failOnError: true,
    ...
    .step('test-packages', {
      ...
      command: [
        ...
        `mkdir -p "$repo_root/${artifactRoot}"`,
        'cd "$repo_root"',
        `npm run test > "$repo_root/${artifactRoot}/test.log" 2>&1`,
        `printf '%s\\n' 'TEST_PASS' > "$repo_root/${artifactRoot}/test.status"`,
        'echo TEST_PASS',
      ].join(' && '),
      captureOutput: true,
      failOnError: true,
    ...
    .step('package-check-signoff', {
      ...
      command: [
        ...
        `printf '%s\\n' '# Package Checks Signoff' '' 'Commands:' '- npm run typecheck' '- npm run test' '' 'Evidence:' "- ${artifactRoot}/typecheck.log" "- ${artifactRoot}/test.log" '' 'Routing: local|cloud|mcp — all converge on repo-root npm scripts (read-only)' '' 'PACKAGE_CHECKS_READY' > "$repo_root/${artifactRoot}/signoff.md"`,
