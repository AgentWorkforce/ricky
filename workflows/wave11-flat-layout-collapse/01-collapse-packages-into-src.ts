  ...
  const result = await workflow('ricky-wave11-collapse-packages-into-src')
    ...
    .step('preflight', {
      ...
      command: [
        `DIR=${artifactDir}`,
        'mkdir -p "$DIR"',
        'git status --porcelain > "$DIR/git-status.before.txt"',
        'test ! -s "$DIR/git-status.before.txt" || { echo "working tree must be clean before collapse"; exit 1; }',
        ...
        'find packages -type f \\( -name "*.ts" -o -name "*.tsx" \\) | sort > "$DIR/source-inventory.before.txt"',
...
- Every .ts/.tsx file under packages/<layer>/src that needs a new home, one row per file. Generate it with: find packages -type f \\( -name "*.ts" -o -name "*.tsx" \\) | sort and translate the prefix.
