...
6. Read standards/spec inputs deterministically at runtime, including `cat docs/workflows/WORKFLOW_STANDARDS.md`, `cat workflows/shared/WORKFLOW_AUTHORING_RULES.md`, workflow-specific specs, and `cat workflows/meta/spec/generated-workflow-template.md` when generation is in scope.
7. Materialize files to disk, do not rely on stdout.
8. Add deterministic post-edit gates.
9. Include a review stage for significant workflows.
10. Use 80-to-100 validation loops for serious implementation workflows.
11. If generating workflows in bulk, run structural sanity checks and `agent-relay run --dry-run` before sign-off.
12. Import from `@agent-relay/sdk/workflows`, wrap serious workflows in `async function main()`, end with `main().catch(...)`, and call `.run({ cwd: process.cwd() })`.
13. Keep commit/push boundaries explicit and deterministic.
...
For convention-only work, the lighter ladder is: read the plan and standards, edit only declared convention files, verify every target exists, grep for required terms, verify the `CLAUDE.md -> AGENTS.md` symlink when in scope, run scoped change detection against only declared targets, and write or preserve review/signoff artifacts only when the workflow owns them.
