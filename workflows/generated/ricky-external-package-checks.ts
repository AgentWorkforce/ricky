...
async function main() {
  const result = await workflow('ricky-external-package-checks')
    ...
    .step('prepare-context', {
      ...
      command: [
        `mkdir -p '${artifactDir}'`,
        `printf '%s\\n' 'Local: npx tsx ${workflowPath}' 'Cloud: ricky run --cloud --artifact ${workflowPath}' 'MCP: invoke workflow ricky-external-package-checks through the relay broker' > '${artifactDir}/routing.txt'`,
...
End the file with PACKAGE_AUDIT_COMPLETE.`,
    ...
    .step('audit-gate', {
      ...
      command: [
        ...
        `grep -q "PACKAGE_AUDIT_COMPLETE" "${artifactDir}/packages.txt"`,
        `echo "Marker PACKAGE_AUDIT_COMPLETE found"`,
        // Derive required package names from the inventory and verify each appears
        `for pkg in $(node -e "const inv = JSON.parse(require(\\\"fs\\\").readFileSync(\\\"${artifactDir}/external-package-inventory.json\\\",\\\"utf8\\\")); Object.keys({...inv.runtime,...inv.development}).forEach(n=>console.log(n))"); do grep -q "$pkg" "${artifactDir}/packages.txt" || { echo "MISSING: $pkg"; exit 1; }; done`,
...
End the file with EXTERNAL_PACKAGE_CHECKS_READY.`,
    ...
    .step('signoff-gate', {
      ...
      command: [
        `bash -lc 'set -euo pipefail`,
        `grep -q "EXTERNAL_PACKAGE_CHECKS_READY" "${artifactDir}/signoff.md"`,
        `echo "Marker EXTERNAL_PACKAGE_CHECKS_READY found"`,
        `grep -q "exit_code=0" "${artifactDir}/dep-tree.status"`,
        `grep -q "exit_code=0" "${artifactDir}/typecheck.status"`,
        `grep -q "exit_code=0" "${artifactDir}/test.status"`,
