...
Write files to disk, then exit cleanly.`,
    ...
    .step('regression-gate', {
      ...
      command: [
        ...
        'filtered="$(printf "%s\n" "$changed" | sed "/^$/d")"',
        'if test -n "$filtered"; then printf "%s\n" "$filtered" | grep -Eq "^(package\\.json|package-lock\\.json|README\\.md|tsconfig\\.json|vitest\\.config\\.ts|\\.gitignore|workflows/wave5-scale-and-ops/03-align-ricky-package-conventions\\.ts|\\.workflow-artifacts/wave5-scale-and-ops/align-ricky-package-conventions/)"; fi',
        'if test -n "$filtered"; then ! printf "%s\n" "$filtered" | grep -Ev "^(package\\.json|package-lock\\.json|README\\.md|tsconfig\\.json|vitest\\.config\\.ts|\\.gitignore|workflows/wave5-scale-and-ops/03-align-ricky-package-conventions\\.ts|\\.workflow-artifacts/wave5-scale-and-ops/align-ricky-package-conventions/)"; fi',
