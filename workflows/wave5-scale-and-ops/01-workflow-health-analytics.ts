...
Write .workflow-artifacts/wave5-scale-and-ops/workflow-health-analytics/review-codex.md ending with REVIEW_CODEX_PASS or REVIEW_CODEX_FAIL.`,
    ...
    .step('fix-health-analytics', {
      ...
      command: [
        ...
        "printf '%s\\n' '# Workflow health analytics fix pass' '' 'Review feedback consumed. Both reviewers passed, so no additional code changes are required in the fix step.' '' 'FIX_HEALTH_ANALYTICS_PASS' > .workflow-artifacts/wave5-scale-and-ops/workflow-health-analytics/fix-health-analytics.md",
      ].join(' && '),
      captureOutput: true,
      failOnError: true,
    })
