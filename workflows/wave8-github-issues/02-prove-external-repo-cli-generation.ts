...
Write .workflow-artifacts/wave8-github-issues/prove-external-repo-cli-generation/fix-loop.md ending with EXTERNAL_REPO_PROOF_FIX_LOOP_COMPLETE.`,
    ...
    .step('external-repo-smoke', {
      ...
      command: [
        ...
        'mkdir -p "$TMP_REPO/node_modules/.bin"',
        'printf "%s\\n" "#!/usr/bin/env node" "import { access } from \'node:fs/promises\';" "import { constants } from \'node:fs\';" "const [, , command, workflowPath] = process.argv;" "if (command !== \'run\' || !workflowPath) { console.error(\'[fixture-agent-relay] expected usage: agent-relay run <workflow-path>\'); process.exit(1); }" "try { await access(workflowPath, constants.F_OK); console.log(\'[fixture-agent-relay] ran \' + workflowPath); } catch (error) { const message = error instanceof Error ? error.message : String(error); console.error(\'[fixture-agent-relay] missing workflow \' + workflowPath + \': \' + message); process.exit(1); }" > "$TMP_REPO/node_modules/.bin/agent-relay"',
        'chmod +x "$TMP_REPO/node_modules/.bin/agent-relay"',
        '(cd "$TMP_REPO" && npx --no-install agent-relay run "$ARTIFACT") > .workflow-artifacts/wave8-github-issues/prove-external-repo-cli-generation/external-repo-next-command.txt 2>&1',
        'grep -F "[fixture-agent-relay] ran $ARTIFACT" .workflow-artifacts/wave8-github-issues/prove-external-repo-cli-generation/external-repo-next-command.txt',
...
Write .workflow-artifacts/wave8-github-issues/prove-external-repo-cli-generation/final-review.md ending with FINAL_REVIEW_PASS or FINAL_REVIEW_FAIL.`,
    ...
    .step('signoff', {
      ...
      command: [
        ...
        '- npm test --workspace @ricky/cli',
        '- npm test',
        '- external repo smoke with no-install agent-relay fixture',
        '',
        'Acceptance proof:',
