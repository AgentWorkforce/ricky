...
async function main() {
  const result = await workflow('ricky-wave8-fix-cli-artifact-path-and-caller-root')
    ...
    .step('reproduce-path-mismatch', {
      ...
      command: [
        'rm -rf workflows/generated packages/cli/workflows/generated',
        'npm start -- --mode local --spec "generate a workflow for package checks" > .workflow-artifacts/wave8-github-issues/fix-cli-artifact-path-and-caller-root/repro-output.txt 2>&1 || true',
        'test -f workflows/generated/ricky-generate-a-workflow-for-package-checks.ts && echo "REPRO_CURRENTLY_PASSING" || echo "REPRO_CONFIRMED_MISSING_REPO_ROOT_ARTIFACT"',
        'if test -d packages/cli/workflows/generated; then find packages/cli/workflows/generated -type f > .workflow-artifacts/wave8-github-issues/fix-cli-artifact-path-and-caller-root/package-cli-generated-files.txt; fi',
      ].join(' && '),
      captureOutput: true,
      failOnError: false,
    })
...
Write .workflow-artifacts/wave8-github-issues/fix-cli-artifact-path-and-caller-root/fix-loop.md ending with PATH_ROOT_FIX_LOOP_COMPLETE.`,
    ...
    .step('external-path-smoke', {
      ...
      command: [
        ...
        'INIT_CWD="$TMP_REPO" npm start -- --mode local --spec "generate a workflow for package checks" > .workflow-artifacts/wave8-github-issues/fix-cli-artifact-path-and-caller-root/external-path-smoke.txt 2>&1',
        'grep -F "Artifact: workflows/generated/ricky-generate-a-workflow-for-package-checks.ts" .workflow-artifacts/wave8-github-issues/fix-cli-artifact-path-and-caller-root/external-path-smoke.txt',
        'test -f "$TMP_REPO/workflows/generated/ricky-generate-a-workflow-for-package-checks.ts"',
        'test ! -e packages/cli/workflows/generated/ricky-generate-a-workflow-for-package-checks.ts',
        'grep -F "npx --no-install agent-relay run workflows/generated/ricky-generate-a-workflow-for-package-checks.ts" .workflow-artifacts/wave8-github-issues/fix-cli-artifact-path-and-caller-root/external-path-smoke.txt',
