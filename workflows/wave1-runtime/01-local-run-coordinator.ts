...
After editing, stop. Do not modify tests in this step.`,
    ...
    .step('verify-runtime-surface-after-edit', {
      ...
      command: [
        ...
        'changed="$(git diff --name-only; git ls-files --others --exclude-standard)" && if [ -z "$changed" ]; then echo LOCAL_COORDINATOR_RUNTIME_ALREADY_PRESENT; else printf "%s\\n" "$changed" | grep -Eq "^src/runtime/(types|local-coordinator)\\.ts"; fi',
...
Do not broaden scope beyond the coordinator files.`,
    ...
    .step('verify-tests-after-edit', {
      ...
      command: [
        ...
        'changed="$(git diff --name-only; git ls-files --others --exclude-standard)" && if [ -z "$changed" ]; then echo LOCAL_COORDINATOR_TESTS_ALREADY_PRESENT; else printf "%s\\n" "$changed" | grep -Eq "^src/runtime/local-coordinator\\.test\\.ts"; fi',
...
Write .workflow-artifacts/wave1-runtime/local-run-coordinator/final-review-codex.md ending with FINAL_REVIEW_CODEX_PASS or FINAL_REVIEW_CODEX_FAIL.`,
    ...
    .step('regression-gate', {
      ...
      command: [
        ...
        'if [ -n "$changed" ]; then ! printf "%s\\n" "$changed" | grep -Ev "^(src/runtime/(types|local-coordinator|local-coordinator\\.test)\\.ts|src/runtime/index\\.ts|\\.workflow-artifacts/)"; fi',
