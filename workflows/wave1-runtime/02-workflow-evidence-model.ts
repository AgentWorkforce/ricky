...
After editing, stop. Do not modify tests in this step.`,
    ...
    .step('verify-model-after-edit', {
      ...
      command: [
        ...
        'changed="$(git diff --name-only -- src/runtime/evidence/types.ts src/runtime/evidence/capture.ts src/runtime/evidence/index.ts; git ls-files --others --exclude-standard src/runtime/evidence/types.ts src/runtime/evidence/capture.ts src/runtime/evidence/index.ts)" && if [ -z "$changed" ]; then echo WORKFLOW_EVIDENCE_CORE_ALREADY_PRESENT; else printf "%s\\n" "$changed" | grep -Eq "^src/runtime/evidence/(types|capture|index)\\.ts"; fi',
...
- Assertions cover structured fields rather than broad snapshots.`,
    ...
    .step('verify-tests-after-edit', {
      ...
      command: [
        ...
        'changed="$(git diff --name-only -- src/runtime/evidence/capture.test.ts; git ls-files --others --exclude-standard src/runtime/evidence/capture.test.ts)" && if [ -z "$changed" ]; then echo WORKFLOW_EVIDENCE_TESTS_ALREADY_PRESENT; else printf "%s\\n" "$changed" | grep -Eq "^src/runtime/evidence/capture\\.test\\.ts"; fi',
