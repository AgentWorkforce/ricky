  ...
  const result = await workflow('ricky-wave6-close-first-wave-signoff-and-blockers')
    ...
    .step('generate-validation-plan', {
      ...
      command: [
        ...
        "  'shared-models': ['npx vitest run src/runtime/evidence/capture.test.ts'],",
        "  'architecture-docs': ['test -f docs/architecture/ricky-architecture-decision-log.md'],",
        "  'run-coordinator': ['npx vitest run src/runtime/local-coordinator.test.ts'],",
        "  'evidence-model': ['npx vitest run src/runtime/evidence/capture.test.ts'],",
        "  'failure-classification': ['npx vitest run src/runtime/failure/classifier.test.ts'],",
        "  'spec-intake': ['npx vitest run src/product/spec-intake/parser.test.ts'],",
        "  'debugger': ['npx vitest run src/product/specialists/debugger/debugger.test.ts'],",
        "  'validator': ['npx vitest run src/product/specialists/validator/validator.test.ts'],",
        "  'generate-endpoint': ['npx vitest run src/cloud/api/generate-endpoint.test.ts'],",
        "  'local-invocation': ['npx vitest run src/local/entrypoint.test.ts src/local/proof/local-entrypoint-proof.test.ts'],",
        "  'cli-onboarding-ux': ['npx vitest run src/surfaces/cli/cli/onboarding.test.ts src/surfaces/cli/cli/proof/onboarding-proof.test.ts'],",
        "  'cli-onboarding-first-run': ['npx vitest run src/surfaces/cli/cli/proof/onboarding-proof.test.ts'],",
        "  'cli-command-surface': ['npx vitest run src/surfaces/cli/commands/cli-main.test.ts'],",
