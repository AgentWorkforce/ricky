  ...
  const result = await workflow('ricky-wave9-evaluate-local-execution-contract-for-reuse')
    ...
    .step('prepare-context', {
      ...
      command: [
        ...
        'sed -n "1,360p" src/local/entrypoint.ts > .workflow-artifacts/wave9-agent-assistant/evaluate-local-execution-contract-for-reuse/local-entrypoint.snapshot.txt',
        'sed -n "1,260p" src/local/request-normalizer.ts > .workflow-artifacts/wave9-agent-assistant/evaluate-local-execution-contract-for-reuse/request-normalizer.snapshot.txt',
        'sed -n "1,260p" src/surfaces/cli/entrypoint/interactive-cli.ts > .workflow-artifacts/wave9-agent-assistant/evaluate-local-execution-contract-for-reuse/interactive-cli.snapshot.txt',
        'sed -n "1200,1420p" src/local/entrypoint.test.ts > .workflow-artifacts/wave9-agent-assistant/evaluate-local-execution-contract-for-reuse/local-entrypoint-tests.snapshot.txt',
