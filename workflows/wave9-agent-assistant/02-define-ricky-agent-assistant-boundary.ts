  ...
  const result = await workflow('ricky-wave9-define-agent-assistant-boundary')
    ...
    .step('prepare-context', {
      ...
      command: [
        ...
        'sed -n "1,220p" src/surfaces/cli/entrypoint/interactive-cli.ts > .workflow-artifacts/wave9-agent-assistant/define-ricky-agent-assistant-boundary/interactive-cli.snapshot.txt',
        'sed -n "1,260p" src/local/entrypoint.ts > .workflow-artifacts/wave9-agent-assistant/define-ricky-agent-assistant-boundary/local-entrypoint.snapshot.txt',
        'sed -n "1,220p" src/product/generation/skill-loader.ts > .workflow-artifacts/wave9-agent-assistant/define-ricky-agent-assistant-boundary/skill-loader.snapshot.txt',
