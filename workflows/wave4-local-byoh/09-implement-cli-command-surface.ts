  ...
  const result = await workflow('ricky-wave4-implement-cli-command-surface')
    ...
    .step('prepare-artifacts', {
      ...
      command: [
        ...
        'mkdir -p src/surfaces/cli/commands',
    ...
    .step('read-entrypoint-context', {
      ...
      command: [
        'sed -n "1,320p" src/surfaces/cli/entrypoint/interactive-cli.ts',
        'printf "\n---\n\n"',
        'sed -n "1,220p" src/surfaces/cli/entrypoint/index.ts',
      ].join(' && '),
      captureOutput: true,
      failOnError: true,
    })
    .step('read-cli-context', {
      ...
      command: [
        ...
        'printf "\n---\n\n"',
        'sed -n "1,260p" src/surfaces/cli/cli/mode-selector.ts',
      ].join(' && '),
      captureOutput: true,
      failOnError: true,
    })
...
- src/surfaces/cli/commands/index.ts
- package.json
...
`,
      verification: { type: 'file_exists', value: 'src/surfaces/cli/commands/cli-main.ts' },
    })
    .step('implementation-file-gate', {
      ...
      command: [
        ...
        'test -f src/surfaces/cli/commands/cli-main.test.ts',
        'test -f src/surfaces/cli/commands/index.ts',
        "grep -qE 'help|mode|interactive|runInteractiveCli' src/surfaces/cli/commands/cli-main.ts src/surfaces/cli/commands/cli-main.test.ts",
        "grep -qE 'scripts|bin|start' package.json",
    ...
    .step('final-review-pass-gate', {
      ...
      command: [
        "tail -n 1 .workflow-artifacts/wave4-local-byoh/implement-cli-command-surface/final-review-claude.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CLAUDE_PASS$\"",
        "tail -n 1 .workflow-artifacts/wave4-local-byoh/implement-cli-command-surface/final-review-codex.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CODEX_PASS$\"",
    ...
    .step('regression-gate', {
      ...
      command: [
        'git diff --name-only -- src/surfaces/cli/commands package.json workflows/wave4-local-byoh/09-implement-cli-command-surface.ts > .workflow-artifacts/wave4-local-byoh/implement-cli-command-surface/tracked-changes.txt',
        ...
        '! grep -Ev "^(|src/surfaces/cli/commands/|package\\.json|workflows/wave4-local-byoh/09-implement-cli-command-surface\\.ts|\\.workflow-artifacts/wave4-local-byoh/implement-cli-command-surface/)" .workflow-artifacts/wave4-local-byoh/implement-cli-command-surface/changed.txt',
