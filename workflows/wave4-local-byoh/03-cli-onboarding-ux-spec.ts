...
Write .workflow-artifacts/wave4-local-byoh/cli-onboarding-ux-spec/review-codex.md ending with REVIEW_CODEX_PASS or REVIEW_CODEX_FAIL.`,
    ...
    .step('post-fix-verification-gate', {
      ...
      command: [
        ...
        "const checks = [",
        "  ['banner/ascii', /banner|ASCII/i],",
        "  ['local/BYOH', /local|BYOH/i],",
        "  ['Cloud', /Cloud/i],",
        "  ['acceptance', /acceptance criteria|acceptance/i],",
        "  ['Slack/web', /Slack|web/i],",
        "  ['recovery/failure/unblock', /recovery|failure|unblock/i],",
        "];",
        "const missing = checks.filter(([, pattern]) => !pattern.test(doc)).map(([label]) => label);",
        "if (missing.length) {",
        "  console.error(`CLI UX spec post-fix gate missing required coverage: ${missing.join(', ')}`);",
        "  process.exit(1);",
        "}",
        "console.log('CLI_UX_SPEC_POST_FIX_GATE_PASS');",
        "NODE",
      ].join('\n'),
      captureOutput: true,
      failOnError: true,
...
Note that this workflow intentionally uses a single Claude final-review path because the current non-interactive Codex reviewer runtime has been observed to hang in this slice after producing artifacts, including the active overnight run.`,
    ...
    .step('regression-gate', {
      ...
      command: [
        ...
        "if [ -z \"$changed\" ]; then",
        "  echo CLI_UX_SPEC_REGRESSION_GATE_PASS_NOOP",
        "else",
        ...
        "  echo CLI_UX_SPEC_REGRESSION_GATE_PASS_CHANGED",
        "fi",
      ].join('\n'),
      captureOutput: true,
      failOnError: true,
