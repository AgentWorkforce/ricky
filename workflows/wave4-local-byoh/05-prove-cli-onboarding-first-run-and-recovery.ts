...
async function main() {
  const result = await workflow('ricky-wave4-prove-cli-onboarding-first-run-and-recovery')
    ...
    .step('final-review-pass-gate', {
      ...
      command: [
        "tail -n 1 .workflow-artifacts/wave4-local-byoh/prove-cli-onboarding-first-run-and-recovery/final-review-claude.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CLAUDE_PASS$\"",
        "tail -n 1 .workflow-artifacts/wave4-local-byoh/prove-cli-onboarding-first-run-and-recovery/final-review-codex.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CODEX_PASS$\"",
