...
Write the files to disk, then exit cleanly.`,
    ...
    .step('final-review-pass-gate', {
      ...
      command: [
        "tail -n 1 .workflow-artifacts/wave4-local-byoh/prove-local-spec-handoff-and-artifact-return/final-review-claude.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CLAUDE_PASS$\"",
        "tail -n 1 .workflow-artifacts/wave4-local-byoh/prove-local-spec-handoff-and-artifact-return/final-review-codex.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CODEX_PASS$\"",
    ...
    .step('regression-gate', {
      ...
      command: [
        ...
        'if [[ -z "$changed" ]]; then echo LOCAL_BYOH_PROOF_REGRESSION_GATE_PASS_NOOP; exit 0; fi',
