...
- src/local/context-detector.ts
- src/local/skill-loader.ts
- src/local/types.ts
...
Verification:
- Context detector must identify repo root, package metadata where available, workflow folders, and relevant local skill paths.
- Entrypoint must accept spec or workflow artifact input from CLI/MCP and return local artifacts/log/run metadata.
- Local/BYOH execution must remain first-class and explicit.
- Tests must prove local routing and artifact/log response contracts.
...
- context-detector.ts should detect repo context, workflow folders, package metadata, local skill locations, and whether local agent-relay is available or needs guidance.
- skill-loader.ts should load local skill descriptors or return deterministic empty results with warnings.
- types.ts and index.ts should export public local invocation contracts.
...
- Preserve the distinction between local workflow generation and Cloud workflow generation.`,
    ...
    .step('post-implementation-file-gate', {
      ...
      command: [
        ...
        'test -f src/local/context-detector.ts',
        'test -f src/local/skill-loader.ts',
        'test -f src/local/types.ts',
        'test -f src/local/index.ts',
        'grep -q "context\\|detect\\|repo" src/local/context-detector.ts',
        'grep -q "skill" src/local/skill-loader.ts src/local/types.ts',
        'grep -q "local\\|BYOH\\|agent-relay" src/local/entrypoint.ts src/local/types.ts',
...
Deliverables:
- src/local/entrypoint.test.ts should cover repo context detection, local skill discovery, spec input routing, workflow artifact input routing, local artifact/log response shape, and environment warning behavior.
...
Verification:
- Use temporary directories or injected filesystem/process adapters where appropriate.
- Tests must prove local/BYOH mode is explicit and not routed through Cloud by default.`,
    ...
    .step('post-test-file-gate', {
      ...
      command: [
        ...
        'grep -q "context\\|repo\\|detect" src/local/entrypoint.test.ts src/local/context-detector.ts',
        'grep -q "skill" src/local/entrypoint.test.ts src/local/skill-loader.ts',
        'grep -q "local\\|BYOH\\|agent-relay" src/local/entrypoint.test.ts src/local/entrypoint.ts',
        'grep -q "artifact\\|log\\|warning" src/local/entrypoint.test.ts src/local/types.ts',
...
Write .workflow-artifacts/wave4-local-entrypoint/review-codex.md ending with REVIEW_CODEX_PASS or REVIEW_CODEX_FAIL.`,
    ...
    .step('review-verdict-gate', {
      ...
      command: [
        'grep -Eq "REVIEW_CLAUDE_PASS$|REVIEW_CLAUDE_FAIL$" .workflow-artifacts/wave4-local-entrypoint/review-claude.md',
        'grep -Eq "REVIEW_CODEX_PASS$|REVIEW_CODEX_FAIL$" .workflow-artifacts/wave4-local-entrypoint/review-codex.md',
...
- Run deterministic gates after changes.`,
    ...
    .step('regression-gate', {
      ...
      command: [
        'npx tsc --noEmit',
        'changed="$(git diff --name-only; git ls-files --others --exclude-standard)" && printf "%s\n" "$changed" | grep -Eq "^(src/local/|src/runtime/|src/product/spec-intake/|src/product/generation/)"',
