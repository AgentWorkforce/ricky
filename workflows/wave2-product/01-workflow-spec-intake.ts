...
After editing, stop. Do not modify tests in this step.`,
    ...
    .step('verify-core-after-edit', {
      ...
      command: [
        ...
        'changed="$({ git diff --name-only; git diff --cached --name-only; git ls-files --others --exclude-standard; } | sed "/^$/d")" && if [ -n "$changed" ]; then printf "%s\\n" "$changed" | grep -Eq "^src/product/spec-intake/(types|parser|normalizer|router|index)\\.ts$"; else git diff --quiet && git diff --cached --quiet && test -z "$(git ls-files --others --exclude-standard)"; fi',
...
- Tests stay deterministic and do not call external services.`,
    ...
    .step('verify-tests-after-edit', {
      ...
      command: [
        ...
        'changed="$({ git diff --name-only; git diff --cached --name-only; git ls-files --others --exclude-standard; } | sed "/^$/d")" && if [ -n "$changed" ]; then printf "%s\\n" "$changed" | grep -Eq "^src/product/spec-intake/parser\\.test\\.ts$"; else git diff --quiet && git diff --cached --quiet && test -z "$(git ls-files --others --exclude-standard)"; fi',
...
Write .workflow-artifacts/wave2-product/workflow-spec-intake/final-review-codex.md ending with FINAL_REVIEW_CODEX_PASS or FINAL_REVIEW_CODEX_FAIL.`,
    ...
    .step('final-review-pass-gate', {
      ...
      command: [
        "tail -n 1 .workflow-artifacts/wave2-product/workflow-spec-intake/final-review-claude.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CLAUDE_PASS$\"",
        "tail -n 1 .workflow-artifacts/wave2-product/workflow-spec-intake/final-review-codex.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CODEX_PASS$\"",
        'echo SPEC_INTAKE_FINAL_REVIEW_PASS',
      ].join(' && '),
      captureOutput: true,
      failOnError: true,
    })
    ...
    .step('final-hard-gate', {
      ...
      command: 'npx vitest run src/product/spec-intake/',
      captureOutput: true,
      failOnError: true,
    })
    ...
    .step('regression-gate', {
      ...
      command: [
        'npx vitest run',
        'changed="$({ git diff --name-only; git diff --cached --name-only; git ls-files --others --exclude-standard; } | sed "/^$/d")" && if [ -n "$changed" ]; then printf "%s\\n" "$changed" | grep -Eq "^src/product/spec-intake/"; ! printf "%s\\n" "$changed" | grep -Ev "^(src/product/spec-intake/|\\.workflow-artifacts/)"; else git diff --quiet && git diff --cached --quiet && test -z "$(git ls-files --others --exclude-standard)"; fi',
