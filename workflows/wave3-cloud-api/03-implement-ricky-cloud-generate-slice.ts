...
Write the files to disk, then exit cleanly.`,
    ...
    .step('final-review-pass-gate', {
      ...
      command: [
        "tail -n 1 .workflow-artifacts/wave3-cloud-api/implement-ricky-cloud-generate-slice/final-review-claude.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CLAUDE_PASS$\"",
        "tail -n 1 .workflow-artifacts/wave3-cloud-api/implement-ricky-cloud-generate-slice/final-review-codex.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CODEX_PASS$\"",
    ...
    .step('regression-gate', {
      ...
      command: [
        ...
        'test -f src/cloud/api/generate-endpoint.test.ts',
        'test -f src/cloud/api/index.ts',
        'test -d .workflow-artifacts/wave3-cloud-api/implement-ricky-cloud-generate-slice',
        'changed="$(git diff --name-only; git diff --cached --name-only; git ls-files --others --exclude-standard)"',
        '! printf "%s\n" "$changed" | grep -v "^$" | grep -Ev "^(src/cloud/api/|workflows/wave3-cloud-api/03-implement-ricky-cloud-generate-slice\\.ts|\\.workflow-artifacts/wave3-cloud-api/implement-ricky-cloud-generate-slice/)" >/dev/null',
