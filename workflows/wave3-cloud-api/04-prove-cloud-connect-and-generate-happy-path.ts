...
Write the files to disk, then exit cleanly.`,
    ...
    .step('final-review-pass-gate', {
      ...
      command: [
        "tail -n 1 .workflow-artifacts/wave3-cloud-api/prove-cloud-connect-and-generate-happy-path/final-review-claude.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CLAUDE_PASS$\"",
        "tail -n 1 .workflow-artifacts/wave3-cloud-api/prove-cloud-connect-and-generate-happy-path/final-review-codex.md | tr -d '[:space:]*' | grep -Eq \"^FINAL_REVIEW_CODEX_PASS$\"",
    ...
    .step('regression-gate', {
      ...
      command: [
        'test -s .workflow-artifacts/wave3-cloud-api/prove-cloud-connect-and-generate-happy-path/final-review-claude.md',
        'test -s .workflow-artifacts/wave3-cloud-api/prove-cloud-connect-and-generate-happy-path/final-review-codex.md',
        'scoped_tracked="$(git diff --name-only -- src/cloud/api workflows/wave3-cloud-api/04-prove-cloud-connect-and-generate-happy-path.ts)"',
        'scoped_untracked="$(git ls-files --others --exclude-standard -- src/cloud/api workflows/wave3-cloud-api/04-prove-cloud-connect-and-generate-happy-path.ts)"',
        'changed="$(printf "%s\\n%s" "$scoped_tracked" "$scoped_untracked" | sed "/^$/d")"',
        'if [ -n "$changed" ]; then ! printf "%s\\n" "$changed" | grep -Ev "^(src/cloud/api/|workflows/wave3-cloud-api/04-prove-cloud-connect-and-generate-happy-path\\.ts)$"; fi',
