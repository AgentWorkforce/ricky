  ...
  const result = await workflow('ricky-wave10-prove-agent-assistant-live-product-path')
    ...
    .step('external-cli-live-path', {
      ...
      command: [
        ...
        'env = os.environ.copy()',
        'env["INIT_CWD"] = tmp_repo',
        'try:',
        '    run = subprocess.run(command, cwd=tmp_repo, env=env, capture_output=True, text=True, timeout=45)',
        '    with open(output_path, "w", encoding="utf-8") as fh:',
        '        fh.write(run.stdout)',
        '        if run.stderr:',
        '            fh.write(run.stderr)',
        '    with open(exit_path, "w", encoding="utf-8") as fh:',
        '        fh.write(str(run.returncode))',
        ...
        '        },',
        '        "evidence": {',
        '            "outcome_summary": "Ricky CLI generate-and-run timed out after 45s in an external temp repo.",',
        '            "logs": {',
        '                "tail": [line for line in ((error.stdout or "") + (error.stderr or "")).splitlines()[-40:]],',
        '                "truncated": False',
        '            },',
        '            "side_effects": {',
        '                "files_written": [],',
        '                "commands_invoked": [" ".join(command)]',
        '            },',
        '            "assertions": [{',
        '                "name": "external_cli_timeout",',
        '                "status": "fail",',
        '                "detail": "Ricky CLI generate-and-run timed out after 45s in an external temp repo."',
        '            }]',
        '        }',
        '    }]',
        '    with open(output_path, "w", encoding="utf-8") as fh:',
        ...
        '    with open(exit_path, "w", encoding="utf-8") as fh:',
        '        fh.write("124")',
        '    print("EXTERNAL_CLI_TIMEOUT", file=sys.stderr)',
        '    sys.exit(124)',
        'PY',
