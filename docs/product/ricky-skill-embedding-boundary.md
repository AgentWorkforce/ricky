...
The generated workflow writes boundary metadata during deterministic context preparation, and Ricky validates that metadata in process while generating the artifact. The metadata records `generation_time_only`, names the loaded skills, includes the `generation_selection`, `generation_loading`, and applicable `generation_rendering` stages, and records effects such as `workflow_contract` and `validation_gates`. The artifact carries the skill boundary forward as metadata; it does not prove runtime agents load skills.
