...
- **Review-fix-signoff loop:** When Ricky reads a spec and generates a workflow, the rendered workflow must apply the `review-fix-signoff-loop` skill: fresh-context reviewer iterations, a dual Claude+Codex verdict contract, repairable post-fix re-review, and a final signoff artifact under `.workflow-artifacts/`. The skill is loaded as a default workflow-generation skill and the generator emits `generation_rendering` evidence for it alongside `choosing-swarm-patterns`, `writing-agent-relay-workflows`, and `relay-80-100-workflow`.
