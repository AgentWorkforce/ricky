...
But the speed can suffer when the renderer skips three decisions it should make deliberately, and the cost shows up in the produced workflow:

1. **Skill matching must stay registry-backed.** The earlier hardcoded `AVAILABLE_PREREQUISITES` path has been replaced; the spec requirement is to preserve matcher behavior against the available skill registry instead of regressing to a static set.
...
3. **Step task descriptions are echoed spec text.** The template renderer's slot-fillers paste the spec's prose into each agent step. For tightly-scoped specs that's enough; for vague specs the agents do all the heavy lifting at runtime, often with under-specified instructions.
   ...
   This one bites in another way: deterministic gates generated from spec acceptance criteria can be too coarse. The current `--version` workflow generated `grep -Eq 'export|function|class|workflow(' dist/ricky.js` as a post-implementation file gate, which fails on intentionally-minimal bin scripts that have none of those tokens. The grep was inferred from "primary artifact" without any behavioral grounding from the spec's actual acceptance text.
...
- **Selection**: pick the top N matches above a confidence threshold (default: 5 skills, ≥ 0.4 confidence). Ties broken by skill update-recency.
- **Output**: `skill-matches.json` records the ranked match list with confidence scores and trigger evidence; `loaded-skills.txt` remains as a human-readable compatibility artifact.
...
Acceptance test for this layer: a spec like the `--version` one would produce a `post-implementation-file-gate` whose command actually verifies `node dist/ricky.js --version | grep -Eq '^ricky [0-9]+\\.[0-9]+\\.[0-9]+$'` (the spec's stated acceptance) instead of a generic source-shape grep.
...
- `skill-matches.json` — ranked match list with confidence scores and trigger evidence.
- `loaded-skills.txt` — human-readable selected skill summary retained for compatibility and audit.
...
End-to-end (manual):
- `ricky --mode local --spec-file specs/cli-version-from-package-json.md` → fast deterministic artifact, current unrefined behavior.
...
- A vague spec that produces a passable scaffold and a tightly-scoped one (like `--version`) that produces a *near-correct* scaffold both produce *behavior-grounded* gates after `--refine`.
